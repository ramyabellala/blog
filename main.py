...
#       login details
        ...
        if user and user.password == password:
            ...
            return '<h1>User doesnot exists </h1>'
...
#   new Post by user
    ...
    if not title_error and not blog_error :
       if input != "" :
            ...
            input_data = Blog(input , entry, owner)
...
def post_blogs():
    add_columns = []
    blogs1 = Blog.query.join(User).add_columns(Blog.id,Blog.title,Blog.blog,User.uname).filter(Blog.owner_id==User.id).all()    
...
#   User Blogs    
        blogs2 = Blog.query.join(User).add_columns(Blog.id,Blog.title,Blog.blog,User.uname).filter(Blog.owner_id==blog_user_id).all()
